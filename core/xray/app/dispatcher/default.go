...
import (
	...
	"github.com/csdfsdffese/ljfxznode/common/counter"
	"github.com/csdfsdffese/ljfxznode/common/rate"
...
func (d *DefaultDispatcher) getLink(ctx context.Context, network net.Network) (*transport.Link, *transport.Link, *limiter.Limiter, error) {
	...
	if user != nil && len(user.Email) > 0 {
		...
		// 对齐官方 Xboard-Node：设备数仅统计 TCP 连接；UDP 会话（含 ss UDP）只做
		// 限数裁决、不计入设备表，因此 noSSUDP 由目标网络是否为 TCP 决定。
...
func (d *DefaultDispatcher) DispatchLink(ctx context.Context, destination net.Destination, outbound *transport.Link) error {
	...
	if user != nil && len(user.Email) > 0 {
		...
		// Speed Limit and Device Limit（UDP 仅裁决不计数，对齐官方 Xboard-Node）
		w, reject := limit.CheckLimit(user.Email,
			sessionInbound.Source.Address.IP().String(),
