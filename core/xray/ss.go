...
func buildSSUser(tag string, userInfo *panel.UserInfo, cypher string, serverKey string) (user *protocol.User) {
	if serverKey == "" {
		...
		// Defensive: the panel-provided uuid must not be shorter than the
		// cipher key length, otherwise the slice below panics.
		if keyLength > 0 && len(userInfo.Uuid) < keyLength {
			keyLength = len(userInfo.Uuid)
		}
